...
function user_not_fully_set_up($user) {
    ...
    // @COREHACK - Additional profile fields check, DPMH 30/04/2012
    global $CFG;
    $pluginfile = $CFG->dirroot.'/local/dlelegacytools/interface/lib.php';
    if (file_exists($pluginfile)) {
        require_once($pluginfile);
        if (lp_additional_profile_fields_not_setup($user)) {
            return true;
        }
    }
    // END COREHACK
...
function send_password_change_confirmation_email($user) {
    ...
    // @COREHACK - added message html for whinging outlook users
    $messagehtml = preg_replace('/(http[s]?:\/\/[^\s$]*)/', '<a href="$1"> $1 </a>', $message);
