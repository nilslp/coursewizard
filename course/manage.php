...
if (!$courses) {
    ...
    foreach ($courses as $acourse) {
        ...
        if (can_delete_course($acourse->id)) {
            // @COREHACK Fix course deletion confirmation to point to /local/dlelegacytools/confirmdelete/confirmcoursedelete.php
...
//            $icons[] = $OUTPUT->action_icon($url, new pix_icon('t/delete', get_string('delete')));
            $url = new moodle_url('/local/dlelegacytools/confirmdelete/confirmcoursedelete.php', array('id' => $acourse->id));
            $icons[] = $OUTPUT->action_icon($url, new pix_icon('t/delete', get_string('delete')));
            // END @COREHACK
        }
