          ...
          /*  if ($option == 'fusion' && (!$oauthenabled || !$oauthcap)) {
            ...
            if(($exportoptions & $code) == $code) {
                if ($has_querystring) {
                    ...
                    $options_html .= "<a href='$current_url&format=$option'><img src='" . $CFG->wwwroot . $pix . "' title='" . get_string('export'.$option,'block_lp_reportbuilder') . "'></a>";
                }
                else {
                    // This is possibly an embedded report without any criteria entered
                    $options_html .= "&nbsp<a href='$current_url?format=$option'><img src='" . $CFG->wwwroot . $pix . "' title='" . get_string('export'.$option,'block_lp_reportbuilder') . "'></a>";                    
                }
            }
        }
...
function pre_process_module_completions() {
    ...
    $sql = "TRUNCATE TABLE {report_builder_module_comp}";
    $DB->execute($sql);
    ...
    $sql = "INSERT INTO {report_builder_module_comp} (
        SELECT
            NULL,
            cs.courseid,
            cs.userid,
    ...
    $sql = "TRUNCATE TABLE {report_builder_scorm_completions}";
    $DB->execute($sql);
    ...
    $sql = "INSERT INTO {report_builder_scorm_completions} (
                SELECT
                    NULL,
                    ss.userid,
                    ss.scormid,
...
function pre_process_course_completion() {
    ...
    $currenttime = time(); 
    // populate module completions table
    $sql = "TRUNCATE TABLE {report_builder_course_comp}";
    $DB->execute($sql);
    ...
    $sql = "INSERT INTO {report_builder_course_comp} 
            (
                `course`,
                `userid`,
                `completion_status`,
    ...
    $sql = "INSERT INTO {report_builder_course_comp} 
            ...
            (
                ...
                FROM {report_builder_course_comp} c
            ) ";
...
//    $sql = "UPDATE {report_builder_course_completion} cc, vw_lp_course_students v
...
//    $sql = "UPDATE {report_builder_course_completion} cc
