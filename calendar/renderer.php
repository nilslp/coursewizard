...
class core_calendar_renderer extends plugin_renderer_base {
    ...
    public function fake_block_threemonths(calendar_information $calendar) {
        // @COREHACK KC 2013-07-11: Customised popup display
        global $CFG;
        ...
        $content .= custom_calendar_get_mini($calendar->courses, $calendar->groups, $calendar->users, $prevmon, $prevyr);
        $content .= html_writer::end_tag('div');
        $content .= html_writer::start_tag('div', array('class'=>'minicalendarblock'));
        $content .= calendar_top_controls('display', array('id' => $calendar->courseid, 'm' => $calendar->month, 'y' => $calendar->year));
        $content .= custom_calendar_get_mini($calendar->courses, $calendar->groups, $calendar->users, $calendar->month, $calendar->year);
        $content .= html_writer::end_tag('div');
        $content .= html_writer::start_tag('div', array('class'=>'minicalendarblock'));
        $content .= calendar_top_controls('display', array('id' => $calendar->courseid, 'm' => $nextmon, 'y' => $nextyr));
        $content .= custom_calendar_get_mini($calendar->courses, $calendar->groups, $calendar->users, $nextmon, $nextyr);
        $content .= html_writer::end_tag('div');
        // COREHACK END
        return $content;
    }
    ...
    public function event(calendar_event $event, $showactions=true) {
        ...
        // @COREHACK DMPH 2013-05-07 Issue #6788 - optionally force login to view calendar events
        if ((int)get_config('local/learningpool', 'forcecalendarlogin')) {
