        ...
        foreach ($users as $user) {
            ...
            if (has_capability('moodle/user:delete', $sitecontext)) {
                if (is_mnet_remote_user($user) or $user->id == $USER->id or is_siteadmin($user)) {
                    ...
                    // @COREHACK DMPH 2012-07-03
                    // redirect user delete to our local handler /local/dlelegacytools/interface/admin/user.php
                    ...
                    $buttons[] = html_writer::link(new moodle_url('/local/dlelegacytools/interface/admin/user.php', array('delete'=>$user->id, 'sesskey'=>sesskey())), html_writer::empty_tag('img', array('src'=>$OUTPUT->pix_url('t/delete'), 'alt'=>$strdelete, 'class'=>'iconsmall')), array('title'=>$strdelete));
                    // END @COREHACK
                }
            }
