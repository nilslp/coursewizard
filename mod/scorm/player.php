...
// @COREHACK -- Adding a new class to distinguish when the SCORM is embedded
if (!$scorm->popup) {
    $PAGE->add_body_class('embedded_scorm');
} 
// END COREHACK
...
if (empty($scorm->popup) || $displaymode=='popup') {
    // @COREHACK - DMPH 2013-05-22: when scorm is embedded, the module status update is out of sync with course redirect, so adding a slight pause to compensate
//    $exitlink = '<a href="'.$CFG->wwwroot.'/course/view.php?id='.$scorm->course.'" title="'.$strexit.'">'.$strexit.'</a> ';
    $exitlink = '<a href="'.$CFG->wwwroot.'/local/learningpool/pause.php?courseid='.$scorm->course.'" title="'.$strexit.'">'.$strexit.'</a> ';
    // END COREHACK
...
// @COREHACK -- Scorm exit button for embedded player
if (!$scorm->popup) {
    echo html_writer::tag('div', $PAGE->button, array('class' => 'scorm-exit-button'));
}
// END COREHACK
// NEW IMS TOC
if (empty($scorm->popup) || $displaymode == 'popup') {
